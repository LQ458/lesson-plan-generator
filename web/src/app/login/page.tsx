...
export default function LoginPage() {
  ...
  // Verify invite code (local verification for simplicity)
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    // Simple local invite code verification with fixed code
    const validInviteCode = "TEACHER2024";
    
    if (inviteCode.trim().toUpperCase() === validInviteCode) {
      setStep("auth");
    } else {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Login successful - redirect to lesson plan
